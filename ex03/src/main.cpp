...
void test_intern() {
  ...
  // Test Make Form
  {
    ...
    try {
      ...
      form = intern.makeForm("robotomy request", "Pawapoke-kun");
      if (form) {
        std::cout << *form << std::endl;
        delete form;
      }
  ...
  // Test Make Form Invalid Form
  {
    printSubtitle("Test Make Form Invalid Form");
    try {
      Intern intern;
      AForm* form = intern.makeForm("invalid form", "home");
      if (form) {
        std::cout << *form << std::endl;
        delete form;
      }
...
void test_from_subject_pdf() {
  printTitle("Test From Subject PDF");
